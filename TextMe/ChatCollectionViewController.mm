...
-(void)viewWillAppear:(BOOL)animated{
    ...
    m_scrolled=false;
    int unread=m_data->getUnreadMsgCount(m_xmppEngine->getToJID().bare());
    if(unread){
        if(m_appDelegate->m_toJid)
            m_data->setMsgStatus(gloox::JID(m_appDelegate->m_toJid.UTF8String).bare(),"" , gloox::MessageEventType::MessageEventDisplayed,true);
...
- (void)viewDidLoad {
    ...
    NSLog(@"padddddddd view %@",self.parentViewController.restorationIdentifier);
    ...
    m_chat=&m_data->getChats(m_xmppEngine->getToJID().bare());
    std::string _name=std::string(m_data->isPartnerExisting(m_xmppEngine->getToJID().bare()).name.c_str());
    NSString * name=[NSString stringWithUTF8String:_name.empty()?m_xmppEngine->getToJID().bare().c_str():_name.c_str()];
   ...
   // [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardShown:) name:UIKeyboardDidShowNotification object:nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardHide:) name:UIKeyboardWillHideNotification object:nil];
     ...
     m_myJID=[NSString stringWithUTF8String:std::get<0>(m_data->getUserInfo()).c_str()];
...
-(CGSize)collectionView:(UICollectionView*) collectionView layout:(nonnull UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(nonnull NSIndexPath *)indexPath{
    ...
    switch (info.type) {
        ...
        case AppData::MESSAGETYPE::FILE_ASSET_ID:{
            ...
            return CGSizeMake(self.collectionView.frame.size.width*.7, 200);
            break;
        }
...
-(void)handleDelShr:(UILongPressGestureRecognizer*)gester{
    ...
    UIAlertAction* share=[UIAlertAction actionWithTitle:@"Share" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [tmpShare setPartnerSharingDataJID:self->m_xmppEngine->getToJID()];
    ...
    UIAlertAction* delet=[UIAlertAction actionWithTitle:@"Delete" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        ...
        if([self->m_textVW isFirstResponder])
            [self->m_textVW resignFirstResponder];
        self->m_xmppEngine->sendMessage(XmppEgine::MESSAGETYPE::DELETE,cell->m_msgID.UTF8String, cell->m_msgID.UTF8String,[self->m_toJid UTF8String]);
        self->m_data->deleteMessage(self->m_xmppEngine->getToJID().bare(),cell->m_msgID.UTF8String);
...
-(void)selectedCellFile:(ChatCollectionViewCell*)chat{
    ...
    if(chat->m_fileType==PHAssetMediaTypeVideo){
        ...
        [imageMgr requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeDefault options:imgRqst resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            ...
            [imageViewCon setToJID:[NSString stringWithUTF8String:self->m_xmppEngine->getToJID().bare().c_str()]];
            ...
            if( ([_id isEqualToString:tmpMsg.msg_ID] && event>=16) || (  [_id isEqualToString:tmpMsg.msg_ID]  && tmpMsg.msg_event<=event) ){
                ...
                if(event==gloox::MessageEventOffline){
                    ...
                    if(tmpMsg.type==AppData::MESSAGETYPE::FILE_ASSET_ID){
                        ...
                        [self sendNotification:fileURL messageID:tmpMsg.msg_ID subscriber:[NSString stringWithUTF8String:self->m_xmppEngine->getToJID().username().c_str()]  userJID:[NSString stringWithUTF8String:self->m_xmppEngine->getMyJID().bare().c_str()] msgType:AppData::MESSAGETYPE::FILE_URL];

                    }else{
                        [self sendNotification:tmpMsg.msg messageID:tmpMsg.msg_ID subscriber:[NSString stringWithUTF8String:self->m_xmppEngine->getToJID().username().c_str()]  userJID:[NSString stringWithUTF8String:self->m_xmppEngine->getMyJID().bare().c_str()] msgType:tmpMsg.type];
...
-(void) tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if([indexPath row] == ((NSIndexPath*)[[tableView indexPathsForVisibleRows] lastObject]).row){
        ...
        if(!m_scrolled){
            auto tmp=m_data->getChats(m_xmppEngine->getToJID().bare());
...
-(void)handleChatRosterPresence:(const gloox::RosterItem &)item resource:(const std::string &)resource presence:(const gloox::Presence::PresenceType)presence message:(const std::string &)msg{
    if(item.jidJID().bareJID()==m_appDelegate->m_toJid.UTF8String ){
        if(presence==gloox::Presence::Available){
            m_partnerPresenceBarItem.title=@"Online";
            ...
            m_xmppEngine-> m_lastActivity->query(item.jidJID());
...
-(void)handleChatMessage:(const gloox::Message &)msg session:(gloox::MessageSession *)session{
    ...
    if(chat_txt_type==AppData::MESSAGETYPE::DELETE){
        std::cout<<"hey hey this message was deleted"<<std::endl;
        if(gloox::JID()==m_xmppEngine->getToJID())
            return;
        auto &tmp=m_data->getChats(m_xmppEngine->getToJID().bare());
        ...
        for(int i=(int)m_chat->size()-1;i>-1;i--){
                ...
                if(tmpCell){
                    ...
                    if(tmpCell->m_msgID.UTF8String==msg.subject().substr(2,msg.subject().size()-1)){
                        std::cout<<"kddkkdkdkdkkdkdkkdkdkkdkdkkkktk4    "<<tmpCell->m_msgID.UTF8String<<"\t"<<msg.body()<<"     "<<m_chat->size()<<std::endl;
                        [tmpCell->m_msgEvent setText:@"⁄⁄"];
                        break;
                    }
       ...
       // m_xmppEngine->m_myevent->raiseMessageEvent(msg.from(), gloox::MessageEventDisplayed, msg.id());
        ...
        m_chat=&m_data->getChats(m_xmppEngine->getToJID().bare());
        std::cout<<"chat number of rolls :"<<(*m_chat).size()-1<<std::endl;
        NSIndexPath *tmpIndex=[NSIndexPath indexPathForItem:(*m_chat).size()-1 inSection:0];
        if([NSThread isMainThread]){
           
            
            //[self.collectionView beginUpdates];
            [self.collectionView insertItemsAtIndexPaths:@[tmpIndex]];
            [self.collectionView  scrollToItemAtIndexPath:tmpIndex atScrollPosition:UICollectionViewScrollPositionBottom animated:YES];
...
-(void)sendNotification:(NSString *)msg  messageID:(NSString*) msgID subscriber:(NSString*) number userJID:(NSString*)jid msgType:(AppData::MESSAGETYPE)type{
    //if(![token UTF8String] || ![number UTF8String])
    // return;
 
    NSURLComponents * comp=[[NSURLComponents alloc]init];
    [comp setScheme:@"http"];
    [comp setHost:@"104.154.81.77"];
    [comp setPath:@"/push"];
    [comp setPort:@9898];
    NSURLQueryItem *service=[NSURLQueryItem queryItemWithName:@"service" value:@"GuysApp"];
    NSURLQueryItem* subscriber=[NSURLQueryItem queryItemWithName:@"subscriber" value:number];
     //msgType is the same  Noti. Category
    NSString *apnsStr=[NSString stringWithFormat:@"{\"aps\" : {\"category\":\"%i\",\"badge\":1, \"alert\" :{\"body\":\"%@\"},\"mutable-content\":1,\"sound\":\"chime.aiff\"},\"jid\":\"%@\",\"msgID\":\"%@\"}",type,msg,jid,msgID];
...
-(void)handleChatDeleted{
    if(gloox::JID()!=m_xmppEngine->getToJID())
       m_data->getChats(m_xmppEngine->getToJID().bare()).clear();
...
-(void)sendMessage:(id)sender{
    if([((UIButton*)sender).currentTitle isEqualToString:@"Cancel"]){
        if([m_recorder cancel]){
            [sender setTitle:@"Send" forState:UIControlStateNormal];
            [sender setImage:[UIImage imageNamed:@"send"] forState:UIControlStateNormal];
            [m_micBTN setImage:[UIImage imageNamed:@"Record"] forState:UIControlStateNormal];
    ...
    if([[m_textVW text] length]==0){
        return;
    }
    bool allCharIsSpace=true;
    std::string msg=m_textVW.text.UTF8String;
    for(char c:msg){
        if(c!=*(" ")){
    ...
    m_xmppEngine->sendMessage(XmppEgine::MESSAGETYPE::TEXT,std::string([m_textVW.text UTF8String]),m_xmppEngine->getID(),m_appDelegate->m_toJid.UTF8String);
    m_textVW.text=@"";
}
- (void)makeRecording:(id)sender {
    if(m_recording){
        ...
        [m_micBTN setImage:[UIImage imageNamed:@"Record"] forState:UIControlStateNormal];
        [self.navigationItem setTitle:currenctPartnerName];
        [m_sendBTN setTitle:@"Send" forState:UIControlStateNormal];
        [m_sendBTN setImage:[UIImage imageNamed:@"send"] forState:UIControlStateNormal];
        ...
        [m_micBTN setImage:[UIImage imageNamed:@"Recording"] forState:UIControlStateNormal];
        [self.navigationItem setTitle:@"Recording"];
        [m_sendBTN setTitle:@"Cancel" forState:UIControlStateNormal];
        [m_sendBTN setImage:[UIImage imageNamed:@"CancelRecording"] forState:UIControlStateNormal];
    }
}
...
-(void)setChatMenuConstrains{
   ...
   // [[self.collectionView.heightAnchor constraintEqualToConstant:80] setActive:YES];
    ...
    [m_photoBTN.layer setCornerRadius:5];
    [m_micBTN.layer setCornerRadius:5];
    [m_sendBTN.layer setCornerRadius:5];
    [m_textVW.layer setCornerRadius:5];
    [m_photoBTN.layer setMasksToBounds:YES];
    [m_micBTN.layer setMasksToBounds:YES];
    [m_sendBTN.layer setMasksToBounds:YES];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<UIImagePickerControllerInfoKey,id> *)info{
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        if(self->m_xmppEngine->clientConnect())
...
-(void)handleLastActivityResult:(const gloox::JID &)jid timeInSec:(long)seconds statusMsg:(const std::string &)status{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        m_partnerPresenceBarItem.title=[dateFormatter stringFromDate:now];
    });
