...
XmppEgine::XmppEgine(id delegate,const std::string user_jid,const std::string pwd, const std::string server,bool forContactRes):
    ...
    try {
       ...
       // m_client->registerIqHandler(this, gloox::StanzaExtensionType::ExMz)
       
        m_client->registerPresenceHandler(this);
        
        m_client->logInstance().registerLogHandler(gloox::LogLevelDebug, gloox::LogAreaAll, this);
        //m_myevent=new MyMessageEvent(m_client);
        //m_myevent->registerMessageEventHandler(this);
       // m_client->logInstance().registerLogHandler(gloox::LogLevelDebug, gloox::LogAreaAll, m_myevent);
        ...
        // m_client->disco()->addFeature(gloox::XMLNS_AMP);
        
        //m_client->setTls(gloox::TLSRequired);
        //m_client->setPort(5223);
       // m_tcpClient=new gloox::ConnectionTCPClient(m_client->logInstance(),server,5222);
       // m_tls=new gloox::ConnectionTLS(m_client,m_tcpClient,m_client->logInstance());
        //m_client->setConnectionImpl(m_tls);
        registrationMode=false;
        //file
        //m_sockManager=new gloox::SOCKS5BytestreamManager(m_client->logInstance(),this);
        m_fileTransferManager=new HttpFileUpload(gloox::JID(server),m_client);
        ...
        m_client->disco()->setVersion( "Nosy", gloox::GLOOX_VERSION, "ios" );
        m_client->disco()->setIdentity( "client", "Nosy" );
     // m_sockServer=new gloox::SOCKS5BytestreamServer(m_client->logInstance(),5443,m_server);
      ...
      //  m_siprofilteFT->registerSOCKS5BytestreamServer(m_sockServer);
       ...
       // m_listeningToStreams=false;
        m_lastActivity=new MyLastActivity(m_client);
        m_lastActivity->registerLastActivityHandler(this);
        m_lastActivity->resetIdleTimer();
            connect();
...
void XmppEgine::handleVCard(const gloox::JID& jid,const gloox::VCard* vcard){
    std::cout<<jid.username()<<" cccc adding "<<jid.bare()<<std::endl;
...
inline void XmppEgine::handleRoster( const gloox::Roster& roster ){
    ...
    //std::thread t([this,roster]{
        m_lastActivity->resetIdleTimer();
        for(auto ros:roster){
            
              m_lastActivity->query(ros.second->jidJID());
   ...
   //});
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        std::cout<<"\t rosterss \t"<<roster.size()<<std::endl;
    });
...
inline void XmppEgine::handleMessageSession( gloox::MessageSession* session ) {
   ...
   auto it=m_sessions.begin();
    ...
    for (; it!=m_sessions.end(); ++it) {
        if(std::get<0>(*it)->target().bare()==session->target().bare()){
            found=true;
...
inline void XmppEgine::handleMessage( const gloox::Message& msg, gloox::MessageSession* session  ){
  ...
  //  if(msg.body().size()&& msg.subtype()==gloox::Message::Chat && msg.from().bare()!=getMyJID().bare())
    //    m_myevent->raiseMessageEvent(msg.from(), gloox::MessageEventDelivered,msg.id());
...
void      XmppEgine::handleItemDeletion (const std::string &id, const gloox::JID &service, const std::string &node, const gloox::PubSub::ItemList &itemListclass , const class Error *error){
