...
- (IBAction)createAccount:(id)sender {
    if([m_phoneNumber isFirstResponder])
      [ m_phoneNumber resignFirstResponder];
    if(![m_callingCode.text length]){
        UIAlertController *cont=[UIAlertController alertControllerWithTitle:@"Guys!" message:@"Select Your Country" preferredStyle:UIAlertControllerStyleAlert];
         [cont addAction:[UIAlertAction actionWithTitle:@"Continue" style:UIAlertActionStyleDestructive handler:nil]];
        [self presentViewController:cont animated:YES completion:nil];
        return;
    }else if ([m_phoneNumber.text length]<9){
    
        UIAlertController *cont=[UIAlertController alertControllerWithTitle:@"Guys!" message:@"Number is incorrect" preferredStyle:UIAlertControllerStyleAlert];
        ...
        for (int idx=0;idx<m_phoneNumber.text.length;idx++) {
            if((idx==0 ||idx==1) && [m_phoneNumber.text characterAtIndex:idx]=='0')
                continue;
            if((idx==0 ||idx==1) && [m_phoneNumber.text characterAtIndex:idx]=='1')
                continue;
         ...
         m_phoneNumber.text=[NSString stringWithUTF8String:tmpNumber.c_str()];
    }
     ...
     NSURLSessionTask *codeTast=[[NSURLSession sharedSession] dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
         ...
         if(! [[[json objectForKey:@"messages"][0] objectForKey:@"accepted"] boolValue]){
             dispatch_async(dispatch_get_main_queue(), ^{
                 UIAlertController *cont=[UIAlertController alertControllerWithTitle:@"Guys!" message:[@"Incorrect Number : " stringByAppendingString:number] preferredStyle:UIAlertControllerStyleAlert];
             ...
             dispatch_async(dispatch_get_main_queue(), ^{
                 UIAlertController *cont=[UIAlertController alertControllerWithTitle:@"Guys!" message:@"Code is sent to your number" preferredStyle:UIAlertControllerStyleAlert];
...
*/
                    ...
                    self->m_xmppEngine=new XmppEgine(self,"18.119.14.179");
        ...
        if(regResult==0 || regResult==2){
                ...
                if(self->m_data){
                   // std::string token=std::get<3>(self->m_data->getUserInfo());
                    ...
                    self->m_data->setAccount(std::string(self->m_currentCallingCode)+std::string(self->m_number+"@18.119.14.179"), std::string(self->m_currentCallingCode),std::string());
            ...
            [self  dismissViewControllerAnimated:YES completion:^{
                ...
                if(self->m_xmppEngine){
                    ...
                    self->m_delegate->m_xmppEngine=nullptr;
                ...
                if(self->m_data){
                    delete self->m_data;
                    self->m_data=nullptr;
                    self->m_delegate->m_data=nullptr;
                }
                
               
          
               // self->m_delegate->m_xmppEngine=new XmppEgine(self->m_delegate->m_viewControllar, std::get<0>(self->m_data->getUserInfo()) +"/iphone",gloox::JID(std::get<0>(self->m_data->getUserInfo())).username(),"18.119.14.179",false);
