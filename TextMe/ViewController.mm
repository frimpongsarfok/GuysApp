...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data{
   ...
   if([m_appDelegate->shareToArray count]){
	  ...
	  for(NSArray* fileSendingInfo in m_sendingFileInfos){
		   NSLog(@"finish -- %@ -- \t%@",fileSendingInfo[0],fileSendingInfo[1]);
		 ...
		 m_data->setFileURLInfo([fileSendingInfo[0] UTF8String],[fileSendingInfo[1] UTF8String]);
		   m_xmppEngine->sendMessage(XmppEgine::MESSAGETYPE::FILE_URL,[fileSendingInfo[1] UTF8String],[fileSendingInfo[0] UTF8String],fromTo.second);
...
- (void)URLSession:(NSURLSession *)session task:(nonnull NSURLSessionTask *)task didReceiveChallenge:(nonnull NSURLAuthenticationChallenge *)challenge completionHandler:(nonnull void (^)(NSURLSessionAuthChallengeDisposition, NSURLCredential * _Nullable))completionHandler{
   ...
   if([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]){
	  if([challenge.protectionSpace.host isEqualToString:@"18.119.14.179"]){
...
-(void)updateContactNPartnerInfo:(NSArray*)data{
   try {
	  ...
	  std::string cc=std::get<1>(m_data->getUserInfo());
	  ...
	  for (int n=0;n< [tmpData count];n++) {
		 ...
		 [self cleanPartner:data];
		 self->m_data->sortContact();
	    // if(m_appDelegate->m_contactViewController)
			//[((ContactViewController*)m_appDelegate->m_contactViewController).m_emailTableView reloadData];
	
		 
		 if(m_xmppEngine->clientConnect()){

			
			//std::thread([self,jid]{
			
			//future update
			//create contact resource connection for only contact update (vcard etc)
			//to inprove the speed on internet connection.
			
			//this thread sleep for 5 seconds to prevent interned speed overload
			//when many jid vcard are search for at time
			std::thread([self,data]{
			   try{
				  AppData::PartnersType cont=self->m_data->contact();
				  for (auto contact:cont) {
					 std::this_thread::sleep_for(std::chrono::milliseconds(300));
					 if(m_xmppEngine && m_xmppEngine->clientConnect())
						 self->m_xmppEngine->fetchVCard(gloox::JID(contact.jid));
...
-(void)cleanPartner:(NSArray*)contactData{
   //delete from partner if not in contact list and has no chat record
   
  // std::thread t1([self,contactData]{
	  std::string cc=std::get<1>(m_data->getUserInfo());
	  for (auto partner : m_data->getPartners()){
		 bool found=NO;
		 NSString * partnerNumber=[NSString stringWithUTF8String:gloox::JID(partner.jid).username().c_str()];
		 for (int n=0;n<[contactData count];n++) {
			NSString *contactNumber=[self getInternationalNumber:contactData[n][0] callingCode:  [NSString stringWithUTF8String:cc.c_str()]];
			if([contactNumber isEqualToString:partnerNumber]){
			   found=YES;
			   break;
			}
		 }
		 if(!found){
			if(partner.inroster){
			   m_data->setPartnerInContact(partner.jid, false);
			}else {
			 //  m_data->deletePartner(partner.jid);
			}
		 }
	  }
   //});
...
-(void)viewDidLoad {
   ...
   m_server=@"18.119.14.179";
   m_resource=@"iphone";
   m_serverNResource=@"@18.119.14.179/iphone";
   ...
   try {
	  ...
	  @autoreleasepool {
		 ...
		 m_xmppEngine=new XmppEgine(self, std::get<0>(m_data->getUserInfo()) +"/iphone",gloox::JID(std::get<0>(m_data->getUserInfo())).username(),m_server.UTF8String,false);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
	  ...
	  NSString *name=[NSString stringWithUTF8String:m_data->chatRoster()[indexPath.row].name.c_str()];;
	  NSString *number=[NSString stringWithUTF8String:gloox::JID(m_data->chatRoster()[indexPath.row].jid).username().c_str()];;
	  cell->m_jid=m_data->chatRoster()[indexPath.row].jid;
	  //std::cout<<"%%%%% "<<m_data->roster()[indexPath.row].name<<" : "<<m_data->roster()[indexPath.row].jid<<std::endl;
	  ...
	  [m_deletePartnerGest setMinimumPressDuration:1.5];
	  [cell addGestureRecognizer:m_deletePartnerGest];

	  NSString *p=[NSString stringWithUTF8String:m_data->chatRoster()[indexPath.row].photo.c_str()];;
	  if(p){
		 NSData *photo=[[NSData alloc]initWithBase64EncodedString:p options:0];
		 
		
		 if(!m_data->chatRoster()[indexPath.row].photo.empty()){
	  ...
	  int numUnread=m_data->getUnreadMsgCount(gloox::JID(m_data->chatRoster()[indexPath.row].jid));
...
-(void)handleSelfVCard:(const gloox::VCard *)card{
   ...
   if(!card->jabberid().size()){
	  m_xmppEngine->storeVCard(tmpVC);
	  return;
   }
   std::string photo=std::get<2>(m_data->getUserInfo());
   if(!card->photo().binval.size()&& !photo.size())
	  return;
   ...
   if(vcardFirstline!=dbFirstline && dbFirstline.size()){
	  std::string code=std::get<1>(m_data->getUserInfo());
	  tmpVC->setPhoto("image/jpeg",photo);
	  m_xmppEngine->storeVCard(tmpVC);
   }
   std::string token=std::get<3>(m_data->getUserInfo());
    std::cout<<"\nself vcard received 2  "<<token<<std::endl;
   //register device token on server
    if(!m_newDeviceToken)
	   return;
   NSString * oldToken=[NSString stringWithUTF8String:token.c_str()];
  // if([oldToken length]){
	 // [self unregisterDevice:oldToken subscriber: [NSString stringWithUTF8String:gloox::JID(std::get<0>(self->m_data->getUserInfo())).username().c_str()]];
   //}
  // sleep(1);
  
   if(![oldToken isEqualToString:m_newDeviceToken]){
	  	m_data->setDeviceToken(m_newDeviceToken.UTF8String);
	  	[self registerDevice:m_newDeviceToken subscriber: [NSString stringWithUTF8String:gloox::JID(std::get<0>(self->m_data->getUserInfo())).username().c_str()]];
	   NSLog(@"??????? %@\n",m_newDeviceToken);
   }
		...
		// dispatch_sync(dispatch_get_main_queue(), ^{
		 [self->m_rosterTableView beginUpdates];
			...
			if(photo){
			   ...
			   [partnerCell->m_photos setImage:[UIImage systemImageNamed:@"person"]];
			}
...
- (IBAction)accountType:(id)sender {
   ...
   if(control.selectedSegmentIndex){
	  [m_newContactName setHidden:YES];
	  [_m_newContNameLabel setHidden:YES];
	  [_m_newContNumberLabel setText:@"Gmail Account"];
	  [m_newContNumber setPlaceholder:@"e.g. : kanta@gmail.com"];
   }else{
	  [_m_newContNumberLabel setText:@"Phone Number"];
	  [m_newContNumber setPlaceholder:@"e.g. : 0245145586"];
	  [_m_newContNameLabel setHidden:NO];
	  [m_newContactName setHidden:NO];
   }
}

-(void)handlePresence:(const gloox::Presence& )presence{
   
   if(presence.from().bare()==std::get<0>(m_data->getUserInfo()) && presence.presence()==gloox::Presence::Available){
		  ...
		  for (auto chat : m_data->getAllUnsendChats()) {
			 ...
			 switch (type) {
				...
				case AppData::MESSAGETYPE::YOUTUBE_LINK:{
				   m_xmppEngine->sendMessage(XmppEgine::MESSAGETYPE::YOUTUBE_LINK,txt.UTF8String,_id,_to,YES);
...
-(NSString*)getInternationalNumber:(NSString*)number callingCode:(NSString *)code{
   if(![code length] ||! number.length)
	  return nil;
   NSString *tmpIntNum=[[NSString alloc]init];
   ...
   //start trimming number if it start with + 0 or 00 or 01
   if([tmp characterAtIndex:0]=='0')
	  tmp=[tmp substringFromIndex:1];
   if([tmp characterAtIndex:0]=='0')
	   tmp=[tmp substringFromIndex:1];
   //if length of number is longer than 10
   // it means the number already have callingcode
   if([tmp length]>10)
   	   return tmp;
   //calling code is store in mailer in AppData

   tmpIntNum=[code characterAtIndex:0]=='+'?[code substringFromIndex:1]:code;
	 ...
	 AppData::MESSAGETYPE chat_txt_type=(AppData::MESSAGETYPE)std::atoi(msg.subject().substr(0,1).c_str());
	  ...
	  switch (chat_txt_type) {
		 case AppData::MESSAGETYPE::TEXT:{
			...
			}else if(m_xmppEngine->clientConnect() && msg.from().bare()==m_xmppEngine->getMyJID().bare()){
	  ...
	  [self shuffleRow:msg.from().bare()==m_xmppEngine->getMyJID().bare()?msg.to().bare():msg.from().bare()];
...
-(void)partnerNewMsgCount:(gloox::JID)jid{
   for(int row=0;row<m_data->roster().size();row++){
...
-(void)viewPartnerChat:(gloox::JID )jid userName:(NSString*)name{  
   ...
   std::cout<<"99838383838  "<<jid.bare()<<"  "<<m_xmppEngine->getToJID().bare()<<std::endl;
...
-(void)shuffleRow:(const gloox::JID )userJID{
   if(m_data->chatRoster().size()<=1)
	  return;
   for(int i=0;i<m_data->chatRoster().size();++i){
	  ...
	  if(gloox::JID(cell1->m_jid).bare()==userJID.bare() && i!=0){
		...
		// dispatch_sync (dispatch_get_main_queue(), ^{
			...
			//for(int i=1;i<self->m_data->roster().size();++i){
			  // 	 [self->m_rosterTableView moveRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0] toIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
			 //}
...
-(BOOL)storeMessage:(NSString*)_id  message:(NSString *)msg  toJIDBare:(NSString*) to fromJIDBare:(NSString*) from  textType:(AppData::MESSAGETYPE) txt_type {
   if(![msg length])
	  return false;
   std::string _from=!([from length])?std::get<0>(m_data->getUserInfo()):([from UTF8String]);
   std::cout<<"messsssage idd "<<[_id UTF8String]<<std::endl;
   auto tmp=m_data->insertNewMessage(_id.UTF8String,_from,[to UTF8String],[msg UTF8String],txt_type);
   if(!tmp.second){
	  std::cout<<" is in roster\t"<<m_data->roster().size()<<std::endl;
	 // [m_rosterTableView beginUpdates];
	 // [m_rosterTableView insertRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:m_data->roster().size()-1 inSection:0]] withRowAnimation:UITableViewRowAnimationTop];
	 // [m_rosterTableView endUpdates];
...
-(NSString *)groupPath{
   ...
   NSDictionary *msgData=[[NSDictionary alloc]initWithContentsOfFile:dataPath];
  if([[NSFileManager defaultManager]fileExistsAtPath:dataPath])
   	NSLog(@"dic : %@ %@",dataPath,msgData);
   return dataPath;
}
